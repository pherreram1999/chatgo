...
const host = `ws://${import.meta.env.VITE_ENDPOINT}/ws`
...
<template>
    <div>
        <div v-if="step === Steps.Login" class="h-screen flex justify-center items-center dark:bg-gray-900">
            <div>
                <form @submit.prevent="conectar"
                      class="bg-white p-6 rounded shadow-2xl text-shadow-black text-shadow-lg/60 rounded-2xl border border-stone-200 w-[26rem] dark:shadow-indigo-950 dark:border-0 dark:bg-gray-800 dark:text-white dark:text-shadow-indigo-500 dark:text-shadow-lg/60">
                  ...
                  color === 'orange' ? 'bg-orange-500' : ''
                    ...
                    <div class="flex justify-end">
                        <button type="submit"
                                class="text-white bg-neutral-900 hover:bg-black dark:bg-black rounded px-6 py-2 dark:hover:bg-neutral-900 transition-colors duration-200">
                            Conectar
                        </button>
                    </div>
                </form>
        ...
        <div class="h-screen flex dark:bg-gray-900" v-else>
            <div class="mx-auto flex justify flex-col bg-indigo-500 mt-10 mb-10 rounded-xl shadow-2xl w-full max-w-2xl
      overflow-auto dark:shadow-indigo-950 dark:border-0 dark:bg-gray-800 dark:text-white">
                <div class="bg-indigo-500 text-4xl pt-4 pb-4 pl-10 rounded-t-xl font-bold text-zinc-200
         dark:bg-gray-800 dark:text-shadow-lg/60 dark:text-shadow-indigo-500 flex justify-between items-center">
                    <span>Chatroom</span> <span class="text-lg px-4">Equipo Websocket</span>
                </div>
                <div class="overflow-auto flex flex-col bg-white dark:bg-gray-900 p-5 h-[60vh]"
                     ref="containerRef"></div>
                <div
                    class="flex-shrink-0 px-4 p-8 min-h-[100px] overflow-auto flex flex-col bg-indigo-500 dark:bg-gray-800">
                    <form @submit.prevent="sendMessage"
                          class="border rounded-lg p-2 border-stone-300 flex bg-white text-black">
                        <input type="text"
                               autofocus
                               v-model="message"
